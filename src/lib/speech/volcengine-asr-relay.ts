...
export async function createRealtimeAsrSession(uid?: string) {
  ...
  ws.on("open", () => {
    ...
    const initFallbackMs = Number(process.env.VOLCENGINE_ASR_INIT_FALLBACK_MS ?? "70");
    ...
    const resolvedFallbackMs = Number.isFinite(initFallbackMs) ? Math.max(0, initFallbackMs) : 70;
