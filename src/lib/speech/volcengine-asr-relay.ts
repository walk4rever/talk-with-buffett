...
export async function createRealtimeAsrSession(uid?: string) {
  ...
  ws.on("open", () => {
    ...
    const guardedDelayMs = Number(process.env.VOLCENGINE_ASR_INIT_GUARDED_DELAY_MS ?? "85");
    ...
    const resolvedGuardedDelayMs = Number.isFinite(guardedDelayMs) ? Math.max(0, guardedDelayMs) : 85;
